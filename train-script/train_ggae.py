...
parser.add_argument("--train_keys", help="train keys", type=input_file, default='data/train_keys.pkl')
...
for epoch in range(num_epochs):
    ...
    for i_batch, sample in enumerate(train_dataloader):
        ...
        pctr_pred, recovered_feature = model.train_model((H, A1, A2, V))
        
        pctr_loss = mse(pctr_pred, Y)
        rec_loss = F.binary_cross_entropy_with_logits(recovered_feature, H, pos_weight=None)
        loss = (1.0-recon_coef)*(pctr_loss) + recon_coef*(rec_loss)
        loss.backward()
        optimizer.step()

        #collect loss, true label and predicted label 
        train_losses.append(loss.data.cpu().detach().numpy())     
        train_true.append(Y.data.cpu().detach().numpy())          
        train_pred.append(pctr_pred.data.cpu().detach().numpy())       

        train_pred_losses.append((pctr_loss.data.cpu().detach().numpy()))
        train_recon_losses.append(rec_loss.data.cpu().detach().numpy())
    ...
    for i_batch, sample in enumerate(test_dataloader):
        ...
        test_pred_losses.append((pctr_loss.data.cpu().detach().numpy()))
        test_recon_losses.append(rec_loss.data.cpu().detach().numpy())

    train_losses = np.mean(np.array(train_losses))
    train_pred_losses = np.mean(np.array(train_pred_losses))
    train_recon_losses = np.mean(np.array(train_recon_losses))

    test_losses = np.mean(np.array(test_losses))
    test_pred_losses = np.mean(np.array(test_pred_losses))
    ...
    if pre_epoch is not None :
        ...
        print(f'train pred loss={train_pred_losses:.3f}   train recon loss={train_recon_losses:.3f}   test pred loss={test_pred_losses:.3f}   test recon loss={test_recon_losses:.3f}')
    else : 
